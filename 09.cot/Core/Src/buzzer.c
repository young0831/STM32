...
void siren(int repeat)
{
	...
	set_buzzer(frequency); // frequency의 주파수로 부저 동작
	HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1); //timer3, ch1로 동작

	for (int i=0; i < repeat; i++) //횟수
	{
		for (int j=0; j < 100; j++)
		{
...
//// 재귀 호출을 이용한 beep 반복 함수
//void beep_recursive(int repeat)
//{
//    if (repeat <= 0)
//        return;
//
//    set_buzzer(2000);  // 2KHz로 부저 설정
//    HAL_Delay(500);
//    noTone();          // 부저 끄기
//    HAL_Delay(200);
//
//    beep_recursive(repeat - 1);
//}
//
...
void buzzer_main()
{
	if(current_state == floor_state && pin_state)
	{
		beep(2);
	}
